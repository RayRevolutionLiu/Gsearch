...
public class s_ASTM
{
    ...
    public int Execute(ISheet IiSheet, string strGUID)
    {
        ...
        sb.Append(@"main_downloadC,main_datetime,main_title,");
        ...
        try
        {
            ...
            for (int i = 5; i < rowcount; i++)
            {
                ...
                if (IiSheet.GetRow(i).Cells[2].ToString().Trim() != "")
                {
                    ...
                    try
                    {
                        yearMonth = IiSheet.GetRow(i).Cells[2].ToString() + "01";//湊成當年當月的一號
                        ...
                        Ipstr = IiSheet.GetRow(i).Cells[4].ToString();
                        documentTitle = IiSheet.GetRow(i).Cells[5].ToString();
                        usr = IiSheet.GetRow(i).Cells[6].ToString();
                        viewCount = Convert.ToInt32(IiSheet.GetRow(i).Cells[7].ToString());
                    }
                    catch
                    {
                        continue;
                    ...
                    oCmd.Parameters["@main_dbtype"].Value = "ASTM";
                    oCmd.Parameters["@main_downloadC"].Value = viewCount;
