...
public class s_ACS
{
    ...
    public int Execute(ISheet IiSheet, string strGUID)
    {
        ...
        try
        {
            ...
            int howManyYears = IiSheet.GetRow(3).Cells.Count - 2;//月份數會不確定,所以需要先判斷CELL數,再去扣掉多餘的
            ...
            for (int i = 5; i < rowcount; i++)
            {
                ...
                for (int j = 1; j < howManyYears + 1; j++)
                {
                    ...
                    if (IiSheet.GetRow(i).Cells[0].ToString().Trim() != "")
                    {
                        try
                        {
                            yearMonth = IiSheet.GetRow(3).Cells[j].ToString();
                            dtYearMonth = Convert.ToDateTime(yearMonth);
                            Ipstr = System.Text.RegularExpressions.Regex.Replace(IiSheet.GetRow(i).Cells[0].ToString(), "0*([0-9]+)", "${1}");
                            ...
                            if (downloadCount != 0)
                            {
                                ...
                                oCmd.Parameters["@main_dbtype"].Value = "ACS";
