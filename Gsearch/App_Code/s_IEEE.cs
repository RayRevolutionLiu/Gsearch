...
public class s_IEEE
{
    ...
    public int Execute(ISheet IiSheet, string strGUID)
    {
        ...
        try
        {
            ...
            int howManyYears = IiSheet.GetRow(5).Cells.Count - 2;//月份數會不確定,所以需要先判斷CELL數,再去扣掉多餘的
            ...
            for (int i = 7; i < rowcount; i++)
            {
                ...
                for (int j = 1; j < howManyYears + 1; j++)
                {
                    ...
                    if (IiSheet.GetRow(i).Cells[0].ToString().Trim() != "")
                    {
                        ...
                        {
                            //日期要這樣抓DateCellValue
                            yearMonth = IiSheet.GetRow(5).GetCell(j).DateCellValue.ToString();
                            ...
                            if (downloadCount != 0)
                            {
                                ...
                                oCmd.Parameters["@main_dbtype"].Value = "IEEE";
