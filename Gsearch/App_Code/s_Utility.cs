...
public class s_Utility
{
    ...
    public int Execute(DataTable dt, string strGUID,string Year,string Month)
    {
        string dbtype = string.Empty;//由於log要用,所以拉出來
        ...
        sb.Append(@"main_viewC,main_downloadC,main_logC,main_indexC,main_datetime,");
        sb.Append(@"main_empno,main_email,main_createdate,");
        sb.Append(@"main_isbn,main_issn");
        //sb.Append(@"");
        sb.Append(@" ) ");
        sb.Append(@" VALUES (");
        sb.Append(@"@main_parentid,@main_ip,@main_dbtype,@main_title,@main_chapter,");
            ...
            for (int i = 1; i < rowcount; i++)
            {
                ...
                if (dt.Rows[i][0].ToString().Trim() != "")
                {
                    ...
                    try
                    {
                        ...
                        downloadCount = dt.Rows[i][3].ToString().Trim() == "" ? 0 : Convert.ToInt32(dt.Rows[i][3].ToString().Trim());
        ...
        finally
        {
            ...
            if (realImportNum != 0)
            {
                ...
                sw.saveLog(realImportNum, strGUID, dbtype, dTime);//做通用log
            }
        }
