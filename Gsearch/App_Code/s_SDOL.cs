...
public class s_SDOL
{
    ...
    public int Execute(ISheet IiSheet, string strGUID)
    {
        ...
        try
        {
            ...
            int howManyYears = IiSheet.GetRow(6).Cells.Count - 1;//月份數會不確定,所以需要先判斷CELL數,再去扣掉多餘的
            ...
            for (int i = 7; i < rowcount; i++)
            {
                ...
                for (int j = 1; j < howManyYears + 1; j++)
                {
                    ...
                    if (IiSheet.GetRow(i).Cells[0].ToString().Trim() != "")
                    {
                        ...
                        {
                            //這裡的日期格式只能這樣處理
                            yearMonth = IiSheet.GetRow(6).GetCell(j).ToString();
                            ...
                            if (downloadCount != 0)
                            {
                                ...
                                oCmd.Parameters["@main_dbtype"].Value = "SDOL";
